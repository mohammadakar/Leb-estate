...
const UpdateListing = () => {
    ...
    const handleImageSubmit = () => {
        if (files.length > 0 && (!formData.imageUrls || (files.length + formData.imageUrls.length < 7))) {
            setUploading(true);
            ...
            Promise.all(promises).then((urls) => {
                setFormData({ ...formData, imageUrls: formData.imageUrls ? formData.imageUrls.concat(urls) : urls });
                setImageUploadError(false);
                setUploading(false)
